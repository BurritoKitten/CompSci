...
public class WordPuzzle {
  ...
  public boolean toBeLabeled(int r, int c, boolean[][] blackBoxes) {
    if (!blackBoxes[r][c] && (r == 0 || blackBoxes[r - 1][c] || c == 0 || blackBoxes[r][c - 1]))
      return true;
    return false;
  }
  ...
  public WordPuzzle(boolean[][] blackBoxes) {
    int h = blackBoxes.length;
    int w = blackBoxes[0].length;
    int num = 0;
    puzzle = new Box[h][w];
    for (int r = 0; r < h; r++) {
      for (int c = 0; c < w; c++) {
        puzzle[r][c] = new Box(blackBoxes[r][c], toBeLabeled(r, c, blackBoxes) ? (++num) : 0);
      }
    }
  }
